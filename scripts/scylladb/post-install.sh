...
# ── 0. Check if ScyllaDB is already running and healthy ──────────────────
# If ScyllaDB is serving CQL on port 9042 and the Raft group is stable,
# NEVER wipe data or restart — this would corrupt the cluster.
if systemctl is-active --quiet scylla-server.service 2>/dev/null; then
    ...
    if [[ -n "${SCYLLA_IP}" ]] && timeout 3 bash -c "echo >/dev/tcp/${SCYLLA_IP}/9042" 2>/dev/null; then
        ...
        echo "[scylladb/post-install] To force reinstall, stop ScyllaDB first: systemctl stop scylla-server"
        exit 0
    fi
...
# ── 4. Wipe data for clean Raft bootstrap ────────────────────────────────
echo "[scylladb/post-install] Wiping data for clean Raft bootstrap..."
